...
`localStorage` 标志：`ttcompass-orientation-granted`（授权成功后写入，用于跳过后续弹窗）。

`startCompass()` 流程：

1. 已授权过 → 直接监听方向事件，**绝不弹按钮**。
2. 有 `DeviceOrientationEvent.requestPermission`（iOS 13+）→ 打开时自动调用：
   - `granted` → 写入 localStorage，继续监听
   - `denied` → 提示「Safari 地址栏点 aA → 网站设置 允许，或清除网站数据后重试」
   - 调用失败或 2.5 秒内没拿到方向数据 → 显示黄色按钮兜底（点击按钮 = 用户手势，重新触发系统权限弹窗）
3. 无 `requestPermission`（Android/桌面）→ 直接监听；9 秒无数据才给错误提示。
4. `updateHeading()` 一旦拿到方向数据就自动隐藏黄色按钮。

**关键限制（不要试图绕过）**：iOS 规定 `requestPermission()` 必须由用户手势触发，所以**首次打开仍会看到一次按钮/权限弹窗**，这是苹果的限制，纯网页无法跳过；授权过一次之后，之后每次打开都直接显示方位、零等待。

## 6. UI 参数（用户对字号/间距有明确偏好，改动前先确认）

- 海拔 `.value`：25px；经纬度 `.coords`：19px；海拔比经纬度调得更多
- 经纬度之间间隔：3 个空格（配合 `white-space: pre` 生效），用户嫌小只加一点点
- 表盘：SVG `viewBox 0 0 400 400`，北东南西大字 + 30° 刻度 + 红白指北针全部静态内联；JS 只旋转 `#dial`（`rotate(-heading)`）
...
- 不要破坏的既有约定：打开即运行（无启动按钮/遮罩）、内联静态 SVG 表盘、`ttcompass-orientation-granted` 权限记忆、Pages 双分支部署链路
